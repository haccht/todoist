...
const bindingHelp = "[q]Quit [?]Help [Enter]Detail"
...
func (u *UI) Init() {
	...
	for i, header := range headers {
		...
		c.SetTextColor(tcell.ColorWhiteSmoke).SetBackgroundColor(tcell.ColorForestGreen)
		if i == len(headers)-1 {
			c.SetExpansion(1)
		}
...
func (u *UI) Popup(title, content string) {
	...
	text.SetDoneFunc(func(key tcell.Key) {
		u.SetFocus(u.table)
		u.pages.SwitchToPage("main")
		u.pages.RemovePage("popup")
	})

	u.pages.AddPage("popup", modal(text, innterWidth+2, 0), true, true)
	u.SetFocus(text)
}

func (u *UI) FormInput(title, text string, callbackFunc func(string)) {
	_, _, width, _ := u.pages.GetRect()
	innterWidth := int(float32(width) * 0.8)
	...
	u.pages.AddPage("popup", modal(input, innterWidth+2, 3), true, true)
	u.SetFocus(input)
}
